...
public class EnchantBladeKirin extends Enchantment {
    ...
    {
        return "enchantment.damage.bladekirin";
    }
    ...
    public void onEntityDamaged(LivingEntity user, Entity target, int level){
    	...
    	if (stack.getItem() instanceof ItemKatanaKirin) {
    		/*
    		 * 亀に追加ダメージ
    		 */
    		if (living instanceof TurtleEntity) {
       			if (user instanceof PlayerEntity) {
    				target.attackEntityFrom(DamageSource.causePlayerDamage((PlayerEntity)user), (float)(damage*10.0D));
    			}else {
    				target.attackEntityFrom(DamageSource.causeMobDamage(user), (float)(damage*10.0D));
    			}
    		}
